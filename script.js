...
// Figurinhas GIF orbitais (HTML overlay sincronizado com a cena 3D)
let orbitStickerObjects = [];
...
function create3DOrbitTexts() {
    ...
    // Órbitas mais lentas, largas e espaçadas para lembrar pequenos planetas.
    const orbitData = [
        [115, 78,  0.15,  0.00125,  5],
        [145, 92,  1.25, -0.00095, -3],
        [175, 108, 2.10,  0.00078,  8],
        [205, 125, 2.85, -0.00068, -7],
        [235, 145, 3.65,  0.00055,  6],
        [260, 160, 4.45, -0.00045, -5],
        [130, 82,  5.20,  0.00105,  4],
        [155, 100, 5.85, -0.00082, -6],
        [190, 118, 0.72,  0.00072,  7],
        [220, 138, 1.75, -0.00058, -4],
        [250, 152, 2.55,  0.00048,  5],
        [280, 172, 3.30, -0.00040, -8],
        [200, 128, 4.95,  0.00062,  3],
        [275, 168, 5.55, -0.00044, -5]
    ];
    ...
    phrases.forEach((phrase, index) => {
        ...
        // Aproximadamente 3–4x maior que a versão anterior.
        const scale = 0.72 + (index % 3) * 0.08;
        sprite.scale.set(100 * scale, 22 * scale, 1);
...
function update3DOrbitTexts(time) {
    ...
    orbitTextObjects.forEach((item, index) => {
        ...
        // Inclinação suave acompanha a trajetória.
        item.sprite.material.rotation =
            Math.sin(a) * 0.10;

        // Brilho bem discreto, sem fazer as palavras piscarem.
        item.sprite.material.opacity =
            0.90 + Math.sin(time * 0.55 + index) * 0.06;
    });
}
...
function createOrbitStickers() {

    const container = document.getElementById("cute-elements");
    if (!container) return;

    const gifs = [
        "assets/teddy.gif",
        "assets/white-bear.gif",
        "assets/heart.gif",
        "assets/smile.gif",
        "assets/pink-heart.gif",
        "assets/bunny.gif"
    ];

    // raio X, raio Z, ângulo, velocidade, altura, tamanho
    const data = [
        [105, 70, 0.35,  0.00145,  8, 54],
        [140, 88, 1.55, -0.00105, -2, 50],
        [178, 110, 2.35, 0.00088,  5, 46],
        [212, 132, 3.10, -0.00070, -6, 56],
        [242, 150, 4.10, 0.00058,  4, 48],
        [270, 166, 5.15, -0.00048, -4, 52]
    ];

    gifs.forEach((src, index) => {
        const img = document.createElement("img");
        img.className = "orbit-gif";
        img.src = src;
        img.alt = "";
        img.draggable = false;
        img.decoding = "async";
        img.loading = "eager";
        img.style.width = data[index][5] + "px";
        img.style.height = data[index][5] + "px";
        container.appendChild(img);

        orbitStickerObjects.push({
            element: img,
            radiusX: data[index][0],
            radiusZ: data[index][1],
            angle: data[index][2],
            speed: data[index][3],
            baseY: data[index][4],
            phase: index * 0.9,
            baseSize: data[index][5]
        });
    });
}


function updateOrbitStickers(time) {

    if (!orbitStickerObjects.length || !camera || !galaxyGroup) return;

    const world = new THREE.Vector3();
    const projected = new THREE.Vector3();

    orbitStickerObjects.forEach((item, index) => {

        item.angle += item.speed;

        const a = item.angle;

        // Posição local na mesma família de órbitas dos textos.
        world.set(
            Math.cos(a) * item.radiusX,
            item.baseY + Math.sin(a * 2 + item.phase) * 5,
            Math.sin(a) * item.radiusZ
        );

        galaxyGroup.localToWorld(world);
        ...
        // Aumenta/diminui naturalmente com a profundidade.
        const distance = camera.position.distanceTo(world);
        const depthScale = Math.max(0.68, Math.min(1.45, 410 / distance));
        const size = item.baseSize * depthScale;

        const float = Math.sin(time * 0.8 + item.phase) * 3;
        const tilt = Math.sin(a * 1.3 + item.phase) * 7;

        item.element.style.left = x + "px";
        item.element.style.top = (y + float) + "px";
        item.element.style.width = size + "px";
        item.element.style.height = size + "px";
        item.element.style.transform =
            "translate(-50%, -50%) rotate(" + tilt + "deg)";

        // Quem estiver mais perto fica um pouco mais evidente.
        item.element.style.opacity =
            String(Math.max(0.48, Math.min(1, 1.15 - distance / 850)));
        item.element.style.zIndex = String(20 + Math.round((1 - projected.z) * 10));
    });
}
